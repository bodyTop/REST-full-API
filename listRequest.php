...
class Request
{
    ...
    private static function getData($request_data)
    {
        if (!empty($request_data['token'])) {
            ...
            $listdata = $redis->lrange('listdata',0,-1);
            if (!$listdata)
            {
                $sql = "SELECT name,head,content,time FROM `user_content` as A JOIN `user` as B ON A.user_id=B.id WHERE is_del='0' and is_public='1' and token='$token' order by time desc";
                $listdata = $model->query($sql);
                $listdata = array_reverse($listdata,true);
                foreach ($listdata as $v){
                    $redis->lPush('listdata',json_encode($v));
                }
            }

            $count = $redis->lsize("listdata");
            $page_data = $base->page($count,$base->pagesize);

            $listdata = $redis->lrange('listdata',$page_data['begin'],$page_data['end']-1);
